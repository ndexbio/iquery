...
import { NDEX_URL } from '../../../api/config'
...
const ProteinInteractionsWidget = (props) => {
  ...
  const handleProteinInteractionsSearch = () => {
    const url = `${NDEX_URL}/viewer/networks/${
      props.network.uuid
    }?query=${props.search.searchResults.validatedGenes.queryGenes.join(
      ' ',
    )}&queryType=${queryType}&maximizeResultView=true`
    ...
    <div>
      <div style={{ height: 'calc(100% - 75px)', padding: '10px', paddingTop: '0px' }}>
        <div style={{ marginTop: '10px', height: 'inherit', overflowY: 'scroll' }}>
          <Typography style={{ marginTop: '8px' }} variant="caption" color="textSecondary">
            Reference
          </Typography>
          <Typography variant="body2" component="div">
            <Linkify>{formatPrimary(reference)}</Linkify>
          </Typography>
          <Typography style={{ marginTop: '10px' }} component="div" variant="caption" color="textSecondary">
            Description
          </Typography>
          <Typography variant="body2" component="div">
            <Linkify>{formatPrimary(description)}</Linkify>
          </Typography>
        </div>
      </div>
      <div
      ...
      >
        <Typography variant="body2" style={{ padding: '10px', overflow: 'hidden', height: '35px' }}>
          Select the type of the Neighborhood Query you wish to run using the dropdown menu below, then click the
          magnifier to run. The query result will be opened in NDEx.
        </Typography>
        <div
          style={{
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
            <div style={{ color: '#f9880e', height: '20px', overflow: 'hidden', flexShrink: 10 }}>
              Click the info icon for details about the available types of queries.
            </div>
          </div>
        </div>
