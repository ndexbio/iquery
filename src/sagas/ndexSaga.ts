...
function* fetchNetwork(action) {
  try {
    ...
    const selectedSource: string = params.selectedSource

    // protein interactions behaves differently than other sources
    // when a user clicks a result, it usually gets the cx and populates
    // props.network but in the protein interactions case, the networks do not
    // exist on the IQuery server so it produces an error.
    // instead of producing an error, instead produce a noop.
    if (selectedSource === 'protein-interactions') {
      yield put({ type: NETWORK_FETCH_NOOP })
    } else {
